...
function patchGameHtml(html: string) {
  ...
  if (!out.includes("lava-menu.js")) {
    out = out.replace("</body>", '<script src="/lava-menu.js?v=warp30"></script><script src="/afro-admin.js?v=warp30"></script></body>');
  }
...
function Index() {
  ...
  useEffect(() => {
    ...
    if (!frame) return;
    let cancelled = false;
    fetch("/game.html?v=warp30", { cache: "no-store" })
      .then((r) => r.text())
      .then((html) => {
