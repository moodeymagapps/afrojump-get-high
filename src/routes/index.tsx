...
function patchGameHtml(html: string) {
  ...
  if (!out.includes("afro-shop.css")) out = out.replace("</head>", '<link rel="stylesheet" href="/afro-shop.css?v=warp50"></head>');
  out = out.replace(
...
export function Index() {
  ...
  useEffect(() => {
    ...
    if (!frame) return;
    let cancelled = false;
    fetch("/game.html?v=warp61", { cache: "no-store" })
      .then((r) => r.text())
      .then((html) => {
